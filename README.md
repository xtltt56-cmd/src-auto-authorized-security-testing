...
> **当前发布基线（2026-09-16）：** 请先阅读 [RELEASE_MANIFEST.md](RELEASE_MANIFEST.md) 与 [TEST_REPORT.md](TEST_REPORT.md) 顶部的本轮验收。当前版本为 `v0.11.1`；本仓库中早于该日期并标注为“历史”的说明仅供追溯，不覆盖当前功能或测试结论。Dashboard 启动时会询问是否允许云端 AI；选择“否”后，本次会话的服务端硬门会拒绝所有远程连接测试。重复启动时不会复用与本次选择不一致的旧授权状态。
...
本次可视化升级新增 `dashboard/` React + Vite 控制台。它通过令牌保护的回环 API 展示
概览、五靶场状态、任务进度、事件详情、授权目标草稿、候选 Finding 和只读报告；页面上的
靶场启动/停止/重置、开始/停止检测、事件选择、草稿保存和报告预览均为真实后端交互。
只有操作者显式点击后，控制台才会管理固定 Docker 靶场或运行只读回环检测；检测结果会写入
项目数据库并生成关联报告。当前版本仍不访问真实外部目标、不调用远程 AI、不把输入域名当成授权，
也不会自动向补天提交报告。
