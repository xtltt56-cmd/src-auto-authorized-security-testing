...
`http://127.0.0.1:4173/` 启动 Vite 预览并打开默认浏览器。仅启动页面不会启动 Docker、访问靶场、
调用远程 AI 或提交任何报告；靶场生命周期和检测必须由操作者在页面中再次点击。若只想启动服务而不打开浏览器，使用
...
3. 未运行的靶场先点击 **启动**，等待健康状态变为“就绪”。“就绪”只表示服务可以访问，并不代表已经检测。
4. 点击该行的 **开始检测**，或进入任务详情后点击 **开始安全检测**。后端会真实访问该靶场的固定 `127.0.0.1:<端口>`，依次显示范围预检、可达性探测、表面发现、候选检测、结果入库、报告生成和检测完成。运行期间可点击 **停止检测**；停止会在下一个安全检查点生效。
5. 完成后，候选计数、运行 ID 和报告入口会关联到该靶场任务；点击 **查看报告** 会打开本次运行对应的人工复核草稿。
6. 页面顶部的 **打开当前任务** 是五靶场聚合任务，和单个靶场详情相互独立。

窄屏设备会把每个靶场重排为一张可点击卡片，避免横向挤压。正式启动器连接真实回环控制 API；只有单元测试和组件预览使用本地夹具。单纯查看页面不会启动容器或执行检测，只有人工点击相应按钮才会操作固定本地靶场。当前检测属于受限候选发现，不等于漏洞确认；它不会访问真实外部网站、调用远程 AI 或自动提交补天报告，结果必须人工复核。
