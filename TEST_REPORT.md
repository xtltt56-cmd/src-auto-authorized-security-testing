...
> **当前发布验收基线：** 本报告顶部的“内置 AI 设置、报告阅读与依赖状态验收（2026-09-15）”适用于 `v0.10.0`。下方所有较早日期的段落均为历史追溯，不覆盖本发布的功能、测试数量或运行结论。旧 ZAP 会话、扫描缓存和机器工件已从 Git 发布内容中排除；当前发布范围见 [RELEASE_MANIFEST.md](RELEASE_MANIFEST.md)。
