...
> **当前开发验收基线：** 本报告顶部的“Dashboard 真实本地检测工作流验收（2026-09-16）”是当前工作区的最新结论；正式发布范围仍以 [RELEASE_MANIFEST.md](RELEASE_MANIFEST.md) 为准。下方所有较早日期的段落均为历史追溯，不覆盖本轮功能、测试数量或运行结论。旧 ZAP 会话、扫描缓存和机器工件继续从 Git 发布内容中排除。
