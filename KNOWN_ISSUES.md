...
> **当前发布读取规则：** 当前基线为 `v0.11.1`，有效状态以本节、[RELEASE_MANIFEST.md](RELEASE_MANIFEST.md) 与 [TEST_REPORT.md](TEST_REPORT.md) 顶部验收为准。下方标为“历史记录”的旧版本状态仅用于追溯，不覆盖当前版本。

## 工作区稳定性升级（待发布，2026-09-17）
...
- 本轮回归：Python `327` 项（`326` 通过、`1` 条件跳过），Dashboard `41/41`，前端覆盖率语句 `75.66%`、分支 `75.08%`、函数 `73.25%`、行 `80.40%`，TypeScript/Lint/生产构建和 PowerShell 解析均通过。该段是工作区候选验收，不代表 GitHub 正式发布已更新。
