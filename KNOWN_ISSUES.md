...
> **当前发布读取规则：** 当前基线为 `v0.11.2`，有效状态以本节、[RELEASE_MANIFEST.md](RELEASE_MANIFEST.md) 与 [TEST_REPORT.md](TEST_REPORT.md) 顶部验收为准。下方标为“历史记录”的旧版本状态仅用于追溯，不覆盖当前版本。

## 2026-09-17 v0.11.2 当前状态

- Dashboard 任务状态现在以项目目录下的脱敏状态日志为准；日志只保存固定靶场任务元数据、计数、运行/报告 ID 和脱敏事件，不保存 API 密钥、Cookie、响应正文或原始命令。
- 服务重启不会恢复排队/运行中的任务；这类记录会显示为“意外中断/失败”，并追加一条人工复核事件，避免页面再次把历史任务显示为“运行中”。已完成检测的运行 ID、报告 ID 和候选计数会保留。
- `STOP_DASHBOARD.bat` / `tools/stop_dashboard.ps1` 只会核对项目身份、回环监听、模块参数和 API 空闲状态后关闭服务；存在活动任务、端口占用者身份不明或项目身份不匹配时拒绝操作，不强制杀进程。
- API 的允许来源由启动器端口显式传入；网页/API 健康接口包含项目身份和进程信息，防止复用其他项目或旧端口服务。连接测试仍受本次启动的云端 AI 硬门和页面二次确认约束。
- 前端失去本地服务时不再显示“未启动”或沿用示例“运行中”状态，而是显示“状态未知/无法确认”，并禁用会改变靶场状态的按钮；恢复服务后才读取真实状态。
- 本轮回归：Python `327` 项（本机 `1` 项、GitHub Runner `3` 项按环境条件跳过，其余通过），Dashboard `41/41`，前端覆盖率语句 `75.66%`、分支 `75.08%`、函数 `73.25%`、行 `80.40%`，TypeScript/Lint/生产构建、PowerShell 解析、Chromium `8/8` 和 GitHub CI 均通过。

## 2026-09-16 v0.11.1 当前状态

- Dashboard 提供总览、本地五靶场、授权草稿、离线范围审阅、候选与报告、云端 AI 设置六个页面。网页通过令牌保护的 `127.0.0.1:4174` 控制接口工作，真实目标执行和补天提交仍需人工单独确认。
- 首页“历史候选记录”与候选页面使用同一项目数据库统计口径；本地靶场生命周期计数不再被当成漏洞检测成绩。
- 从带有报告关联的任务打开“查看报告”时，会直接选择该任务对应的报告；没有报告关联的靶场继续显示“当前靶场尚未生成报告”。
- 报告页面只预览前 65,536 个字符；若预览被截断，可通过受保护的项目白名单接口下载完整脱敏报告。单份完整导出上限为 4 MiB，超过上限时必须在本机项目目录中人工查看。
- AI 设置在密钥或模型存在未保存更改时禁止连接测试，避免把旧密钥或旧模型的成功结果误认为当前输入有效。保存与联网测试仍是两个独立的人工动作。
- Dashboard 启动时先询问本次是否允许云端 AI，并把结果锁定在 API 进程内；拒绝时连接测试由服务端返回 `remote_ai_disabled_for_session`，即使前端误勾选联网测试也不会解密或发送密钥。
- 重复启动会核对已有 API 的会话授权：状态一致才复用；状态缺失或不一致时，只有确认没有运行任务且监听进程确属本项目后才会重启。无法确认时失败关闭，不沿用可能过期的授权。
- Dashboard 已接通人工触发的本地检测工作流：靶场达到“就绪”后，操作者仍须点击“开始检测”；服务端才会对固定回环入口执行只读可达性探测、有限同源表面发现、被动安全响应头检查，并按靶场执行对象授权矩阵或受限 OpenAPI 示例契约检查，随后写入候选、检查点和关联报告。进度、计数、停止请求、运行 ID 与报告 ID 均来自真实后端任务，不再由前端假运行。
- 该工作流是低风险候选检测，不等于完整漏洞确认：它尚未把 ZAP/Nuclei 等深度扫描器统一接入 Dashboard，也不执行破坏性载荷、暴力请求、真实外部目标扫描、远程 AI 或自动提交。所有候选仍须人工复核，不能解释为已确认漏洞或赏金成绩。

## 2026-08-28 可视化控制升级后的实际状态（历史记录）
