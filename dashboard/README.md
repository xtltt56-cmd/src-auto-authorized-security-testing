...
进入左侧的“本地靶场”后，每个靶场名称和对应的“查看任务”按钮都可以打开独立任务页。详情页会显示该靶场的固定回环端口、夹具健康状态、当前阶段、脱敏事件和报告入口；“打开当前任务”仍保留为五靶场聚合任务入口。

界面数据来自本地 fixture，用于演示和验证交互，不等同于当前 Docker 容器真实运行成绩。需要核对容器是否可达时，应以本机 `127.0.0.1` 端口检查和本地验收报告为准；Dashboard 不会因为查看任务页而启动靶场、访问真实目标或调用远程 AI。
