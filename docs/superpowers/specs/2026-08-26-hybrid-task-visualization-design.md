...
- 日期：2026-08-26
- 状态：代码优先可视化升级已获用户明确启动；Figma 冻结门禁由本次修订替换
...
本轮用户明确要求放弃 Figma 冻结并开始可视化升级，因此采用代码优先门禁：现有 `design/frontend-mockups` 和 `docs/design` 中的视觉稿、信息架构与中文文案作为参考输入；React 页面以本规格的令牌、组件清单和交互验收作为唯一实现门槛。Figma 文件和历史导出不删除，也不作为运行时依赖。

代码优先门禁 G0-UI 必须在进入桌面壳接入前通过：

1. 设计令牌、页面状态、组件变体和可访问名称写入 `docs/superpowers/specs/2026-08-26-code-first-visual-upgrade.md`；
2. React + Vite 页面在本地 fixture 下可启动、可点击、可键盘操作；
3. Storybook story 覆盖正常、等待人工、阻止、失败和完成状态；
4. Playwright 在 1366×768、1920×1080 与移动宽度下验证主流程，浏览器控制台无未处理错误；
5. 页面仅访问回环服务或内置夹具，不加载外部 CDN、字体或图标；
6. 视觉偏差以本地截图对照记录，出现无法修复的偏差必须记录为有意差异。
...
本规格的 Figma 冻结要求由 `2026-08-26-code-first-visual-upgrade.md` 和对应实施计划替代；后续仍可把已验收组件同步回 Figma 或 Penpot，但不阻塞本地实现。
